...
export function SignupForm() {
  ...
  return (
    <form className="mt-7 space-y-5" onSubmit={async (event) => {
      ...
      if (!supabase) {
        setSuccess(false);
        setMessage("⚠️ O cadastro não está configurado no ambiente. Configure as variáveis NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY para ativar o recurso. A aplicação pode ser usada sem conta.");
        return;
      }
      ...
      <button
        ...
        disabled={submitting}
        className="secondary-action w-full mt-6 px-4 py-3 text-base font-bold text-center hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="presentation">
          ...
          >
            ...
            <div className="space-y-3 mb-6">
              <button
                type="button"
                onClick={() => handleDemoSignup("demo@test.com", "12345678")}
                className="w-full text-left p-4 rounded-2xl bg-white/50 dark:bg-white/10 hover:bg-white/70 dark:hover:bg-white/20 transition-all border-2 border-transparent hover:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                type="button"
                onClick={() => handleDemoSignup("demo2@test.com", "12345678")}
                className="w-full text-left p-4 rounded-2xl bg-white/50 dark:bg-white/10 hover:bg-white/70 dark:hover:bg-white/20 transition-all border-2 border-transparent hover:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
